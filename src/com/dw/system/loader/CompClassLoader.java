...
public class CompClassLoader extends URLClassLoader
{
	...
	protected static ILogger log = LoggerManager.getLogger(CompClassLoader.class
			.getCanonicalName());
	 ...
	 */
	protected static final String[] triggers = { "javax.servlet.Servlet", // Servlet
																			// API
	 ...
	 * ClassLoader.
	 */
	public CompClassLoader(ClassLoader parent)
	{
	 ...
	 * Java2 delegation model)? If set to <code>false</code>, this class
	 * loader will search its own repositories first, and delegate to the parent
	 * only if the class or resource is not found locally.
	 */
	...
	public void addPermission(String path)
	{
		...
		if (securityManager != null)
		{
			...
			if (path.startsWith("jndi:") || path.startsWith("jar:jndi:"))
			{
				...
				throw new RuntimeException("not support jndi permission!");
			}
			else
			{
...
//			log.debug("modified()");
		...
		if (getJarPath() != null)
		{
			...
			try
			{
				NamingEnumeration enumeration = resources
...
//					log.info("    Additional JARs have been added");
	...
	public String toString()
	{
		...
		StringBuffer sb = new StringBuffer("CompClassLoader\r\n");
		sb.append("  delegate: ");
		sb.append(delegate);
		sb.append("\r\n");
...
//						log
//								.trace("      securityManager.checkPackageDefinition");
					securityManager
...
//			log.debug("      Loaded by " + clazz.getClassLoader());
	 ...
	 * <code>URL</code> refering to it, or <code>null</code> if this
...
//			log.debug("    findResources(" + name + ")");
	 ...
	 * call the <code>getResource()</code> method of the parent class loader,
	 * if any.</li>
...
//			log.debug("  --> Resource not found, returning null");
	 ...
	 * <ul>
	 * <li>Call <code>findLoadedClass(String)</code> to check if the class
	 * has already been loaded. If it has, the same <code>Class</code> object
	 * is returned.</li>
	 * <li>If the <code>delegate</code> property is set to <code>true</code>,
	 ...
	 * <code>resolve</code> flag is <code>true</code>, this method will
	 * then call <code>resolveClass(Class)</code> on the resulting Class
	 * object.
	 * 
	 * @param name
	 *            Name of the class to be loaded
...
//			log.debug("loadClass(" + name + ", " + resolve + ")");
		...
		if (!started)
		{
			...
			{
				log.info("webappClassLoader.stopped" + name, e);
			}
		}
...
//						log.debug("  Loading class from parent");
	 ...
	 * CompClassLoader is for a web application context, add read
				...
				else
				{
					...
					if (null != value)
					{
						...
						if (!loadedByThisOrChild(valueClass))
						{
							...
							{
								log
										.debug("Not setting field "
												+ field.getName()
												+ " to null in object of class "
												+ instance.getClass().getName()
												+ " because the referenced object was of type "
												+ valueClass.getName()
												+ " which was not loaded by this CompClassLoader.");
							}
						}
						else
						{
	...
	/**
	 * Find specified resource in local repositories. This block will execute
	 * under an AccessControl.doPrivilege block.
	 * 
	 * @return the loaded resource, or null if the resource isn't found
	 */
	protected ResourceEntry findResourcernal(File file, String path)
	{
	...
	protected ResourceEntry findResourceInternal(String name, String path)
	{
		...
		if (!started)
		{
			log.info("webappClassLoader.stopped " + name);
			return null;
		}
			...
			for (i = 0; (entry == null) && (i < jarFilesLength); i++)
			{
				...
				if (jarEntry != null)
				{
					...
					if (antiJARLocking && !(path.endsWith(".class")))
					{
						...
						File resourceFile = new File(loaderDir, jarEntry
								.getName());
						if (!resourceFile.exists())
						{
							...
							{
								JarEntry jarEntry2 = (JarEntry) entries
										.nextElement();
