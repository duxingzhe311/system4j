...
/**
 * XmlData结构和文件组合的一种支持对象 目的是支持如下应用情景 既有强数据格式，又有文件内容的数据传输，并且要求尽可能高效和少占用内存
 * 
 * 该类可以支持有效的对数据及文件进行读取传输，反过来也为高效的接收数据文件提供支持 实现方式： 通过定义
 * 
 * @author Jason Zhu
 */
public class XmlDataWithFile
{
	...
	/**
	 * 对接收端提供的回调支持，接收数据及文件端可以通过实现该接口。来高效的提供数据接收
	 * 
	 * @author Jason Zhu
	 */
	public static interface IRecvCallback
	{
		...
		/**
		 * 由于接收数据有可能产生一些需要释放的资源. 实现该方法可以让一些资源得到释放
		 */
	...
	/**
	 * 接收到一个流中的接收回调.该回调在接收数据已组合的方式写入到一个流中.
	 * 
	 * @author Jason Zhu
	 * 
	 */
	...
	/**
	 * 把定义好的结构和本地文件指向的路径进行组合－输出到一个文件中。
	 * 
	 * @param combinefn
	 * @param xdwf
	 */
	...
	/**
	 * 当一个组合文件被处理时,需要首先读取文件头,以便于获取格式数据并且定义里面的文件 该类就是为了方便处理而专门做的类支持
	 * 
	 * @author Jason Zhu
	 */
	...
	/**
	 * 从一个组合文件种获得文件头,该文件头包含原来的内部XmlData及文件信息(文件名,起始位置,长度)
	 * 该方法用来支持获取文件头信息和定位输出文件内容
	 * 
	 * @param inputs
	 * @return
	 * @throws Exception
	...
	/**
	 * 根据数据格式，及本地的文件路径定义对象。 该对象可以为发送数据做准备
	 * 
	 * 之后就可以调用{@see writeToStream}
	 * 
	 * @param xd
	 * @param localpaths
	 */
...
//	}
	...
	/**
	 * 输出仅包含内部数据格式及本地文件路径信息的格式信息 该内容可以作为本地发送临时保存的信息-可以用户回复做后续的处理,如发送
	 * 
	 * @return
	 */
	...
	/**
	 * 计算发送数据的长度--用来支持http方式发送前需要设定的ContentLength
	 * 
	 * @return
	 */
...
//					 call back
	...
	public static void main(String[] args) throws Exception
	{
		...
		XmlDataWithFile xdwf = new XmlDataWithFile(tmpxd, new String[] {
				"E:/tmp/111.htm", "E:/tmp/222啊.swf", "E:/tmp/1/HELLOWIN.WAV",
