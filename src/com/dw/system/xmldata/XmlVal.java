...
//import com.dw.workflow.api.util.BinHexTransfer;
//import com.dw.workflow.api.util.XmlHelper;

/**
 * 数据值，可以转换为xml字符串表示方式，并加入到XmlData对象中 数据值有多种数据类型除了最基本的类型之外（基本类型和对应的数组之外），以后还可以支持
 * java对象类型,type=c:com.xxx.C1 其内部会有不同的java对象来处理 xml schema类型，type=s:schema_url等
 * 其内部会有一个专门的schema对象处理
 * 
 * @author Jason Zhu
 * 
 */
	...
	public XmlVal copyMeWithNewType(String newxv_type)
	{
		...
		if (bArray)
		{
			...
			if(!newxv_type.equals(this.type))
				xvi.getObjectVals() ;//类型不相同,则检查输入的内容是否正确
		}
		else
		{
			xvi.strVal = this.getStrVal();
			if(!newxv_type.equals(this.type))
				xvi.getObjectVal() ;//类型不相同,则检查输入的内容是否正确
		}
...
//	}
	...
	{//类型（1字节，其中数组值+100）
		XmlValType xvt = this.getXmlValType() ;
		if(this.bArray)
			out.write(xvt.val+50) ;//大于50的值表示数组
		else
			out.write(xvt.val) ;
...
return sdf.format(d);
	...
	{
		return Convert.toXmlValDateStr(d);
	}
...
//				Date d = sdf.parse(xmlestr);
	...
	static final int T_PROP = 21;//xmldata 内部的属性值
