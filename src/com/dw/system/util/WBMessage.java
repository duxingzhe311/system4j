...
public class WBMessage
{
	...
	*/
	...
	/** array of boolean, currently not supported */
	...
	*/
	 ...
	 * @exception IllegalArgumentException if msg is dead-locked.
	 */
	public void add (String key , WBMessage msg)
	{
		if (msg == null)
		{
	 ...
	 */
	public boolean contains (WBMessage msg)
	{
		if (msg == this)
			return true ;
		Data data = null ;
	...
	public byte [] pack ()
		throws IOException
	{
		return WBMessage.packToBytes (this) ;
	}
	/**
	 * Pack message Data to bytes array.
	 */
	@SuppressWarnings("deprecation")
	public static byte [] packToBytes (WBMessage msg) 
		throws IOException
	{
		...
		*/
		byte [] encodingBytes = en.getBytes ("UTF-8") ;
	...
	public static WBMessage unpack (byte [] message) throws IOException
	{
		return extractFromStream (new ByteArrayInputStream (message)) ;
	}
	
	public static WBMessage unpack (InputStream in) throws IOException
	{
	 ...
	 */
	public static WBMessage extractFromBytes (byte [] message) throws IOException
	{
		return extractFromStream (new ByteArrayInputStream (message)) ;
	}
	/**
	 ...
	 */
	public static WBMessage extractFromStream (InputStream in)
		throws IOException
	{
		...
		*/
		...
		String encoding = new String (encodingBytes , "UTF-8") ;
		...
		throws IOException
	{
		...
		case MSG :
			...
			bytes = packToBytes ((WBMessage) data.value) ;
		...
		case BEAN :
			...
			WBMessage beanMessage = new WBMessage () ;
			...
			byte [] nameBytes = beanClass.getName ().getBytes ("UTF-8") ;
			int beanSize = bytes.length + 1 + nameBytes.length ;
			out.write (intToBytes (beanSize)) ;
		...
		case HASH_TABLE :
			...
			WBMessage hashMsg = new WBMessage () ;
		...
		throws IOException 
	{
		...
		case MSG :
			...
			data.value = WBMessage.extractFromBytes (msgBytes) ;
		...
		case BEAN_ARRAY :
			...
			try
			{
				beanArray = java.lang.reflect.Array.newInstance (Class.forName (className) , arraySize) ;
		...
		case BEAN :
			...
			int nameLength = readByte (in) ;
			byte nameBytes [] = readBytes (in , nameLength) ;
			
			byte [] bytes = readBytes (in , size - nameLength - 1) ;
			...
			WBMessage beanMessage = WBMessage.unpack (new ByteArrayInputStream (bytes)) ;
			
			
			for (Enumeration e = beanMessage.getKeys () ; e.hasMoreElements () ; )
			{
				String fieldName = (String) e.nextElement () ;
				Object value = beanMessage.get (fieldName) ;
		...
		throws Throwable 
	{
		...
		msg.log () ;
		msg1.log () ;	
		byte [] bytes = WBMessage.packToBytes (msg) ;
		
		WBMessage msg2 = WBMessage.extractFromBytes (bytes) ;
		
		msg2.log () ;
		
		((WBMessage) msg2.get ("message")).log () ;
