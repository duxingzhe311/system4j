...
public class MimeMultipart extends Multipart {
	...
	try {
	    ...
	    boolean tryagained = false;
	    while (true)//((line = lin.readLine()) != null) //modified by me
	    {
	    	line = lin.readLine() ; //modified by me
	    	/////////////////////modified by me
	    	//查找边界
	    	if(line==null)
	    	{
//	    		有可能multipart内容整个是个base64编码,需要重新设定输入流进行分析
		    	if(!tryagained && ds instanceof MimePartDataSource)
		    	{
		    		tryagained = true ;
		    		...
		    		try {
		    			in = ((MimePartDataSource)ds).getInputStreamNoRestrictEncoding();
		    		    if (!(in instanceof ByteArrayInputStream) &&
		    			!(in instanceof BufferedInputStream) &&
		    			!(in instanceof SharedInputStream))
		    			in = new BufferedInputStream(in);
