...
public class BizActionRuntime
{
  ...
  BizActionRtCompilerEnv compEnv = null;

  private BizActionRuntime()
  {
  ...
  public BizActionRuntime(BizManager bm, BizActionRtCompilerEnv env)
  {
    this.bizMgr = bm;
    this.compEnv = env;
  }

  public BizActionRuntime(OutputStream output)
  {
    String tmps = System.getProperties().getProperty("wf.debug").trim();
    if ("ignore".equals(tmps))
      isDebug = 0;
  ...
  private String compile() throws IOException, Exception
  {
    ...
    BizActionCompiler jspc = new BizActionCompiler(
      this.compEnv.getBizAction().getBizPathStr(), this.bizMgr, this.compEnv.getBizAction().getContTreeNodeRoot(), 
